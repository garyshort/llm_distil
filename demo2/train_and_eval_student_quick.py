...
Same as train_and_eval_student.py but with fewer iterations and a smaller eval set,
designed to run in ~2 minutes for live demonstrations. Uses a separate adapter
directory (adapters_qwen25_7b_damage_demo) so it does NOT overwrite the full
adapters in adapters_qwen25_7b_damage.

Requires: pip install "mlx-lm[train]"
Run from this directory: python train_and_eval_student_quick.py
...
# -----------------------------------------------------------------------------
# Configuration (quick demo: fewer iters, separate adapter dir, smaller eval)
# -----------------------------------------------------------------------------
DATA_DIR = Path("data")
MODEL = "mlx-community/Qwen2.5-7B-Instruct-4bit"
ADAPTER_DIR = Path("adapters_qwen25_7b_damage_demo")

# Training (reduced for ~2 min live demo)
ITERS = 25
BATCH_SIZE = 2
LEARNING_RATE = 7e-5
MASK_PROMPT = True
...
def main() -> None:
    ...
    print("=== CONFIG (quick demo) ===")
    print(f"  Model:       {MODEL}")
    print(f"  Data dir:    {data_dir}")
    print(f"  Adapter dir: {adapter_path} (demo - does not overwrite full adapters)")
    print()
    ...
    print("=== EVAL (test set, 10 examples) ===")
    t0 = time.perf_counter()
    eval_on_test(script_dir, model, tokenizer, sampler, max_examples=10)
